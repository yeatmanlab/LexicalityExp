...
% Each stimulus category will be shown 5 times per run.
% and two tasks.


% to make this program work, we need following codes packages
% vistadisp
% knkutility
...
st(:,3) = lstr(oi(1:nstim));  % RZ: do we need this line??Ovewrited by the line below
...
%stimcon = 1,2,3;indexing contrast
...
for ii = 1:size(st,1)
    for jj = 1:size(st,2)
        ...
        tmp(tmp==0)=bg;
        % stack it up
        img(:,:,c) = tmp; % all images
        sc(c) = stimcon(ii,jj); % stim condition (1-3),contrast
        sl(c) = jj; % image category
        stimcat(c) = (jj-1).*4+stimcon(ii,jj);%1~12 catogories
    end
end
stimcat = sl + (sc-1).*4; %stimulus categories,1-4;
...
nblank = 5; %number of blanks in a run
...
for rn = 1:nruns
    ...
    stimorder(rn,:) = insertBlankTrial(horzcat(tmp, zeros(1,nblank)));
end

% Turn stimorder into frameorder denoting blank frames etc that are part of
% a trial
frameorder = makeFrameOrder(stimorder, 10, 10);% 10,10 here basically means 10 frames on and 10 frames off.
